  ...
  return (
    <div className="glass-card-hover p-6 group">
      <div className="w-14 h-14 rounded-xl bg-titan-red/10 flex items-center justify-center mb-5 group-hover:bg-titan-red/20 transition-colors">
        <i className={`fa-solid ${program?.icon || 'fa-dumbbell'} text-2xl text-titan-red`} />
      </div>
      <h3 className="text-xl font-heading font-bold mb-3">{program?.name || 'Unnamed Program'}</h3>
      <p className="text-titan-secondary text-sm mb-4 line-clamp-2">{program?.shortDesc || 'No description available.'}</p>
      <div className="flex items-center justify-between">
        <Link to={`/programs/${program?.slug || program?.id || ''}`} className="text-titan-red text-sm font-semibold uppercase tracking-wider hover:underline flex items-center gap-2">
  ...
  return (
    <div className="glass-card overflow-hidden group hover:shadow-glow transition-all duration-300 hover:-translate-y-1">
      <div className="relative overflow-hidden aspect-[4/5]">
        <img src={trainer?.image || 'https://images.unsplash.com/photo-1567013127542-490d757e51fc?q=80&w=600'} alt={trainer?.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
        ...
        <div className="absolute bottom-4 left-4 right-4">
          ...
          <h3 className="text-xl font-heading font-bold mt-1">{trainer?.name || 'Coach'}</h3>
        </div>
      ...
      <div className="p-5">
        <p className="text-titan-secondary text-sm mb-4 line-clamp-2">{trainer?.shortBio || 'No bio details available.'}</p>
        ...
        <div className="flex items-center justify-between">
          ...
          <Link to={`/trainers/${trainer?.slug || trainer?.id || ''}`} className="btn-glass !px-4 !py-2 !text-xs">
            View Profile
          </Link>
        </div>
      </div>
...
export function PlanCard({ plan, billingPeriod, onAddToCart }) {
  const price = plan?.pricing?.[billingPeriod] ?? 0;
  ...
  return (
    <div className={`glass-card p-8 relative transition-all duration-300 hover:-translate-y-2 hover:shadow-glow ${plan?.popular ? 'border-titan-red/50 ring-1 ring-titan-red/30' : ''}`}>
      ...
      <div className="text-center mb-6">
        <h3 className="text-2xl font-heading font-bold mb-2">{plan?.name || 'Membership Plan'}</h3>
